...
public class ParticipantTests
{
    ...
    {
        yield return [GetDate(9), GetDate(10)];
        yield return [GetDate(9), GetDate(11, 30)];
        yield return [GetDate(10), GetDate(10, 30)];
        yield return [GetDate(10), GetDate(12)];
    }

    public static IEnumerable<object[]> GetAvailableDates()
    {
        yield return [GetDate(8), GetDate(9)];
        yield return [GetDate(8), GetDate(9, 30)];
        yield return [GetDate(11), GetDate(12)];
        yield return [GetDate(12), GetDate(13)];
    }

    private static DateTime GetDate(int hour, int minute = 0, int second = 0)
        => new(2025, 3, 26, hour, minute, second);
}